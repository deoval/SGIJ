...
if (!in_array($_SESSION['user']['cargo'], array('advogado_socio', 'advogado'))) {
    ...
    $campos_da_tabela = array(
        ...
        'Cidade' => 'cidade',
        'Telefone' => 'numero_telefone');
    $tabela = array(TBL_USUARIO, TBL_ADVOGADO, TBL_TELEFONES_USUARIO);
    $condition = "advogado_id_advogado=id and id_telefone_advogado = id and id = " . $_SESSION['user']['id'];
}
...
foreach ($campos_da_tabela as $key => $campos) {
    print "<div style='border:1px solid #ccc;margin:5px;padding:5px;margin-right: 15px;'><span class='capitalize'><b>" . $key . "</b> : </span> " . str_replace('_',' ',$dados[0][$campos]) . "</div>";
}
?>
    </div>
    <div class="left50percent">
