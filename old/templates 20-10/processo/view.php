    ...
    <div class="left50percent">
        ...
        foreach ($campos_da_tabela as $key => $campo) {
            if (in_array($campo, array_keys($campos_juncao))) {
                print "<span>$key : </span> " . $campos_juncao[$campo] . "<br />";
            } else {
                print "<span>$key : </span> " . (in_array($campo, $is_data) ? date_format(date_create($processos[0][$campo]), "d/m/Y") : $processos[0][$campo]) . "<br />";
            }
        }
        ?>
<br /><a class="btn btn-large" href="index.php?r=processo/admin"><?php print BACK; ?></a>
    </div>
    <div class="left50percent">
        <?php
        $pdo = new conectaPDO(); //INICIA CONEXÃO PDO
