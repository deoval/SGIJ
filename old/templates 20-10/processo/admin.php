...
if (!empty($campo_procurado) && !empty($termo_de_busca)) {
  if($campo_procurado == 'data_abertura'){
$termo_de_busca = explode("/",$termo_de_busca);
$termo_de_busca = implode("-",array_reverse($termo_de_busca));
 $condition .= " and date(data_abertura) = date('$termo_de_busca') and Month(data_abertura) = Month('$termo_de_busca') AND Year(data_abertura) = Year('$termo_de_busca')";
}else{
    $condition .= " and $campo_procurado LIKE '%$termo_de_busca%'";
}
}
if ($_GET['date'] != 'all') {
    $condition .= " and data_abertura >= '" . date('Y-m-d') . "'";
}
